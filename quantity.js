import { Unit, units, applyM } from './units';
...
export class Q {
    ...
    add(rhs) {
        ...
        if (lhs.u.isDimensionless())
            lhs = { ...lhs, u: rhs.u.set({ absolute: 0 }) };
        if (rhs.u.isDimensionless())
            rhs = { ...rhs, u: lhs.u.set({ absolute: 0 }) };
...
export function q(num, unit) {
    if (num instanceof Q) {
        if (unit)
            return new Q(q.n, unit);
        return num;
    }
    return new Q(num, unit || u.one);
}

q.zero = q(0, units.one);
